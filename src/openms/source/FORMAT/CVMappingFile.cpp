...
namespace OpenMS
{
  ...
  void CVMappingFile::load(const std::string& filename, CVMappings& cv_mappings, bool strip_namespaces)
  {
  ...
  void CVMappingFile::onStartElement(const char16_t* qname, const Internal::XMLAttributes& attributes)
  {
    ...
    if (tag_ == "CvMappingRule")
    {
      ...
      if (strip_namespaces_)
      {
        ...
        element_path = "";
        for (vector<std::string>::const_iterator it = slash_split.begin(); it != slash_split.end(); ++it)
        {
          if (it->empty())
          {
            continue;
      ...
      std::string lvl = attributeAsString_(attributes, "requirementLevel");
      if (lvl == "MAY")
      {
      ...
      std::string lgc = attributeAsString_(attributes, "cvTermsCombinationLogic");
      if (lgc == "OR")
      {
