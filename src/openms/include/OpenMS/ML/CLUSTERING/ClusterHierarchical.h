...
public:
  ...
  /**
      ...
      @param[in] data Values to be clustered
      @param[in] comparator Similarity functor which returns a similarity in [0, 1] for any pair of values in @p data
      @param[in] clusterer A cluster method implementation, e.g. SingleLinkage or CompleteLinkage. See base class ClusterFunctor.
      @param[out] cluster_tree The vector that will hold the BinaryTreeNodes representing the clustering (for further investigation with the
     ClusterAnalyzer methods)
      @param[in,out] original_distance Precomputed DistanceMatrix holding the pairwise distances of the elements in @p data; if empty or wrong size, it will be re-created using @p comparator
  ...
  */
  ...
  /**
    ...
    @param[in] data vector of @ref PeakSpectrum s to be clustered
    @param[in] comparator a BinnedSpectrumCompareFunctor
    @param[in] sz Bin size for the @ref BinnedSpectrum s
    @param[in] sp Bin spread for the @ref BinnedSpectrum s
    @param[in] offset Bin offset for the @ref BinnedSpectrum s
    @param[in] clusterer a clustermethod implementation, base class ClusterFunctor
    @param[out] cluster_tree Vector of BinaryTreeNodes representing the clustering (for further investigation with the ClusterAnalyzer methods)
    @param[in,out] original_distance The DistanceMatrix holding the pairwise distances of the elements in @p data, will be made newly if given size does not
