...
def package_sdk(sdk: Path, dist: Path, deployed: Path, name: str) -> Path:
    ...
    return extract_sdk(archive, deployed)


def extract_sdk(archive: Path, deployed: Path) -> Path:
    """Check the archive checksum before extracting an SDK for acceptance."""
    with archive.open("rb") as stream:
        checksum = hashlib.file_digest(stream, "sha256").hexdigest()
    if checksum != archive.with_suffix(".gz.sha256").read_text().split()[0]:
        raise ValueError(f"Checksum mismatch: {archive}")
    with tarfile.open(archive) as source:
        source.extractall(deployed, filter="data")
    return deployed / archive.name.removesuffix(".tar.gz")
...
def main() -> None:
    ...
    parser.add_argument("--sdk-run-id", type=int, default=os.environ.get("OPENMS_SDK_RUN_ID") or None,
                        help="Accept an existing CI SDK archive without rebuilding Core")
    args = parser.parse_args()
    source = Path(__file__).resolve().parents[2]
    work = args.work_dir.resolve()
    if args.sdk_run_id is not None and args.sdk_run_id < 1:
        parser.error("--sdk-run-id must be positive")
    ...
    if args.sdk_run_id:
        # Manual acceptance never qualifies a release: release.yml requires a
        # successful branch push run, including its scientific compilation/tests.
        revision = run("sdk-source", ["gh", "run", "view", str(args.sdk_run_id),
                        "--json", "headSha", "--jq", ".headSha"]).strip()
        download = work / "download"
        run("sdk-download", ["gh", "run", "download", str(args.sdk_run_id),
                            "--name", f"sdk-{args.platform}", "--dir", str(download)])
        archives = list(download.glob("*.tar.gz"))
        if len(archives) != 1:
            raise ValueError("Expected exactly one SDK archive")
        deployed = extract_sdk(archives[0], work / "deployed")
        if (deployed / "source-revision.txt").read_text().strip() != revision:
            raise ValueError("SDK source does not match the selected CI run")
    else:
    ...
    if os.environ.get("GITHUB_STEP_SUMMARY"):
        with open(os.environ["GITHUB_STEP_SUMMARY"], "a", encoding="utf-8") as summary:
            scope = "Existing SDK archive acceptance verified" if args.sdk_run_id else "SDK built, scientific tests and archive acceptance passed"
            summary.write(f"### {args.platform}: {scope}\n\n")
            summary.write(f"Linear programming dependency profile: {solver}.\n\n")
